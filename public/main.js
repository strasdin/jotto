$(function () {
    ...
    const $jottoInput = $('.jottoInput');        // Input message input box
    const $guesses1 = $('#guess1');           // Messages area
    ...
    $window.keydown(event => {
        ...
        if (event.which === 13) {
            if (username) {
                if(document.activeElement.className === 'jottoInput'){
                    sendGuess();
                    socket.emit('stop typing');
                    typing = false;
                }else{
    ...
    socket.on('login', (data) => {
        ...
        const message = ' – Welcome to Jotto – ';
        log(message, {
            prepend: true
        });
    ...
    socket.on('jotto start', (data) => {
        alert('Please enter your word in the box on the right.')
    });
    ...
    // Sends a chat message
    const sendGuess = () => {
        let guess = $jottoInput.val();
        // Prevent markup from being injected into the message
        guess = cleanInput(guess);
        // if there is a non-empty message and a socket connection
        let isValid = isValidWord(guess);

        if (guess && connected && isValid) {
            $jottoInput.val('');
            // tell server to execute 'new message' and send along one parameter
    ...
    const addGuess = (data) => {
        let $el = $('<li>').addClass('guess').text(`${data.guess} - ${data.numCorrect}`);
